        ...
        <DropdownMenuItem
          onClick={() => {
            navigator.clipboard.writeText(
              movimiento.idMovimientoEspecial.toString()
            );
            toast("ID copiado");
          }}
        >
          <Copy size={18} />
          <span className="text-sm ml-2">Copiar ID del movimiento</span>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem>
          <Link
            to={`/movimientoespecial/${movimiento.idMovimientoEspecial}`}
            className="flex flex-row items-center gap-2"
          >
            <Pencil size={18} />
            <span className="text-sm">Editar movimiento</span>
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem
          onSelect={(event) => {
          ...
          <AlertDialog open={open} onOpenChange={setOpen}>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Esta acción {""}
                  {movimiento.estado === 1 ? "desactivara" : "activara"} el
                  movimiento de nuestros servidores.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                ...
                <AlertDialogAction
                  onClick={() => {
                    handleChangeStatus(movimiento.idMovimientoEspecial);
                  }}
                  disabled={isLoading}
                  className="gap-2"
                >
